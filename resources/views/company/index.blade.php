            ...
            <table id="datatable-buttons" class="text-center table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
              ...
              <tbody>
                @foreach ($company as $compani)
                  <tr>
                    <td><a href="{{url('company/'.$compani->id)}}">{{ $compani->id }}</a></td>
                    <td>{{ $compani->name }} </td>
                    <td>{{ $compani->status->description }}</td>
                    <td>{{ $compani->email }} </td>
                    <td> <a href="{{url('company/'. $compani->id .'/edit')}}" class="btn btn-outline-primary"> Editar <i class="mdi mdi-square-edit-outline"></i>   </a> </td>
                  </tr>
                @endforeach
              </tbody>
            </table>
